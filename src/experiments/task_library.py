...
def build_task_suite() -> List[TaskSpec]:
    ...
    tasks: List[TaskSpec] = [
        TaskSpec(
            ...
            objective="先快速浏览上传的 scRNA-seq 数据，报告细胞和基因数量，并指出是否包含已知的批次信息",
        ...
        TaskSpec(
            ...
            objective="请完成基础的细胞类型注释，并为每个聚类给出 2-3 个代表性标记基因",
        ...
        TaskSpec(
            ...
            objective="列出最主要的免疫相关簇及其上调标记基因，帮助快速识别免疫亚群",
        ...
        TaskSpec(
            task_id="ssgsea_enrichment_tcell",
            objective="针对富含 T 细胞标记的聚类运行 ssGSEA，说明显著富集的免疫相关通路",
            category="pathway_analysis",
            difficulty="composite",
            expected_keywords=("ssGSEA", "免疫"),
        ),
        TaskSpec(
            task_id="cellphone_communication_core",
            objective="推断主要免疫簇之间的细胞间通信，突出 3-5 个重要的配体-受体对",
            category="cell_communication",
            difficulty="composite",
            expected_keywords=("通信", "配体"),
        ),
        TaskSpec(
            task_id="pseudotime_brief",
            objective="对祖细胞或未分化细胞构建伪时间轨迹，描述早晚期的标记基因变化",
            category="trajectory_analysis",
            difficulty="composite",
            expected_keywords=("伪时间", "轨迹"),
        ),
        TaskSpec(
            task_id="knowledge_validation_rag",
            objective="结合知识库回答：该数据集中与 T 细胞衰竭或活化相关的关键生物学发现",
            category="knowledge_retrieval",
            difficulty="ambiguous",
            expected_keywords=("知识", "T 细胞"),
        ),
        TaskSpec(
        ...
        TaskSpec(
            ...
            objective="如果仅完成了聚类和标注，请建议下一个高价值的分析步骤并说明原因",
        ...
        TaskSpec(
            ...
            objective="请记住研究对象编号 2024-AML-03，后续提及该编号时需保持一致",
            category="memory",
            difficulty="simple",
            requires_dataset=False,
        ...
        TaskSpec(
            ...
            objective="之前记录的研究对象编号是什么？请在回答中再次确认编号并给出当前分析状态",
            category="memory",
            difficulty="ambiguous",
            requires_dataset=False,
