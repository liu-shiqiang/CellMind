...
import { 
  ...
  MoreVertical, ChevronRight, MousePointer2, FileText
} from 'lucide-react';
          ...
          <div>
            ...
            <div className="space-y-1">
              {sessions.map(session => (
                <button key={session.id} onClick={() => loadSession(session)} className={`w-full text-left px-3 py-3 rounded-xl transition-all group overflow-hidden ${currentSessionId === session.id ? 'bg-white border border-slate-200 shadow-sm' : 'hover:bg-slate-100'}`}>
                  <span className={`text-sm font-semibold block truncate ${currentSessionId === session.id ? 'text-blue-600' : 'text-slate-700'}`}>{session.title}</span>
